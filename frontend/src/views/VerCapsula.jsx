...
const VerCapsula = () => {
  ...
  useEffect(() => {
    const fetchArchivos = async () => {
      try {
        const res = await fetch(`http://localhost:3000/api/contents/capsule/${id}`);
        const data = await res.json();
        setArchivos(data);
      } catch (err) {
        setArchivos([]);
                    ...
                    {archivo.Type === 'image' && archivo.Path ? (
                      ...
                      <div className="flex flex-col items-center justify-center w-full h-full">
                        ...
                        <audio controls className="w-full">
                          <source src={`http://localhost:3000/${archivo.Path.replace(/^\/?/, '')}`} />
                          Tu navegador no soporta audio.
                        </audio>
                      </div>
                    ) : archivo.Path ? (
                      <a
                        href={`http://localhost:3000/${archivo.Path.replace(/^\/?/, '')}`}
                        download
