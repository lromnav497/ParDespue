...
const InformacionGeneral = () => {
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      const storedUser = getStoredUser();
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:3000/api/users/${storedUser.id}/password`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
...
const MisSuscripciones = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await fetch(`http://localhost:3000/api/subscriptions/user/${storedUser.id}`, {
  ...
  const handleRenewConfirm = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://localhost:3000/api/subscriptions/renew/${selectedSubscription.id}`, {
        method: 'POST',
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
